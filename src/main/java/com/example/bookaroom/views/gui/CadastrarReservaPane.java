...
import com.example.bookaroom.views.gui.widgets.*;
...
public class CadastrarReservaPane extends GridPane {
    ...
    private void cadastrarReservaAction(ActionEvent actionEvent) {
        ...
        try {
            Reserva novaReserva = App.bookARoomApi().cadastrarReuniao(periodo, sala, assunto);
    ...
    private void filtrarSalasAction(ActionEvent actionEvent) {
        ...
        if(validacaoErr != null){
            ...
            HashMap<Sala, Boolean> salasDisponiveis = App.bookARoomApi().disponibilidadeDeSalas(new Periodo(
                dataField.getText(),
                inicioTimeField.getText(),
                fimTimeField.getText()
        ));
    ...
    private String validarPeriodoInput() {
        try {
            LocalTime inicio = LocalTime.parse(inicioTimeField.getText(), Periodo.FORMATO_HORA);
            LocalTime fim = LocalTime.parse(fimTimeField.getText(), Periodo.FORMATO_HORA);
